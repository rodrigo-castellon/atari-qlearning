...
summary_dir = '/tmp/tflearn_logs/cs/'
...
def build_dqn(num_actions, action_repeat):
    ...
    Building a DQN.
    """
    inputs = tf.placeholder(tf.float32, [None, action_repeat-1, 84, 84])
...
class AtariEnvironment(object):
    ...
    def get_initial_state(self):
        ...
        s_t = np.stack([x_t for i in range(self.action_repeat-1)], axis=0)
    ...
    def step(self, action_index):
        ...
        im_diff=previous_frames-x_t1
        #np.save('diff.npy', im_diff)

        # Pop the oldest frame, add the current frame to the queue
        self.state_buffer.popleft()
        self.state_buffer.append(x_t1)

        return im_diff, r_t, terminal, info
